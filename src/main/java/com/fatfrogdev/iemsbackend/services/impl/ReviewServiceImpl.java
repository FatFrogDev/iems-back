...
public class ReviewServiceImpl implements IReviewService {
    ...
    public ReviewViewDTO saveWithoutImages(ReviewRegisterDTO reviewRegisterDTO, MultipartFile[] images) {
        ReviewEntity reviewEntity = this.buildEntity(reviewRegisterDTO, images);
    ...
    public void saveReviewImages(String reviewId, MultipartFile[] images) throws IOException {
        ...
        for (MultipartFile image : images) {
            if (fileService.hasValidImageExtension(image.getOriginalFilename(), (String) null))
                throw new FileHasNotValidExtensionException("Invalid image extension. Review not saved.");

            if (!fileService.imageHasValidSize(image))
                throw new FileSizeNotValidException(String.format("Image %s size exceeds the limit of 10MB. Review not saved.", image.getOriginalFilename()));
    ...
    public void deleteReviewImages(String reviewId, String imageId) {
        ...
        FileEntity imageToDelete = reviewEntity.getImages().stream()
    ...
    private ReviewEntity buildEntity(ReviewRegisterDTO reviewRegisterDTO, MultipartFile[] images) {
        UserEntity userEntity = this.findUserByUsername(reviewRegisterDTO.getUser());
        ProductEntity productEntity = this.findProductById(reviewRegisterDTO.getProduct());
        ...
        for (int i = 0; i < reviewRegisterDTO.getImages().length; i++) {
            if (fileService.hasValidImageExtension(reviewRegisterDTO.getImages()[i].getOriginalFilename(),(String) null))
                throw new FileHasNotValidExtensionException(String.format("Image %o has an invalid image extension. Review not saved.", i+1));
        }
